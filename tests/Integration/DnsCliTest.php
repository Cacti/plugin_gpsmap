...
it('runs the gpsmap_dns.php worker CLI with the expected exit codes', function () {
	$root   = gpsmap_test_tmpdir();
	$plugin = $root . '/plugins/gpsmap';
	@mkdir($root . '/include', 0700, true);
	copy(dirname(__DIR__, 2) . '/gpsmap_dns.php', $plugin . '/gpsmap_dns.php');
	...
	foreach (['success' => 0, 'fail' => 1, 'throw' => 1] as $mode => $expected) {
		...
		expect($status)->toBe($expected, "dns CLI: $mode worker exit status (output: " . implode(' | ', $output) . ')');
	}

	expect(file_get_contents($root . '/gpsmap-worker.log'))->toContain('simulated worker failure');
});
