...
/*
 ...
 | coverage overlay.                                                       |
 +-------------------------------------------------------------------------+
 | Ported from the fork's rewritten tests/test_polling.php as a small       |
 | number of sequential it() blocks (rather than one expect() per line) so  |
 | the stateful, order-dependent scenario the original author validated is  |
 | preserved exactly, using the shared assert_equal()/assert_true()/etc.    |
 | helpers from bootstrap-unit.php.                                        |
 +-------------------------------------------------------------------------+
*/
...
it('gpsmap_xml_path / gpsmap_write_file: paths, writes and permissions', function () use ($root) {
	assert_equal(
	...
	gpsmap_test_silence(function () use ($root) {
...
it('getTowerIds / createTypeArray', function () {
...
it('gpsmap_marker / createXMLNodes', function () {
	$typeArray = ['10' => 'Tower'];
...
it('region(): full poller pass, KML styles and preservation', function () use ($root) {
	...
	assert_true('artefact: invalid name has no stem', gpsmap_artifact_stem('../escape') === null);
...
it('coverage radius: group geometry, configured radius and coverage toggle', function () {
...
it('single-pass loading: one query and one DNS pass for the whole cycle', function () use ($root) {
	$GLOBALS['gpsmap_stub_rows']['towers'] = [['templateID' => '10']];
	$GLOBALS['gpsmap_stub_rows']['hosts']  = [
...
it('atomic writes: de-duplication, rollback, short writes and pruning', function () use ($root) {
	...
	gpsmap_test_silence(function () use ($xmldir) {
	...
	gpsmap_test_silence(function () use ($root) {
		...
		$blocked = gpsmap_test_tmpdir() . '/plugins/gpsmap/XML/occupied2';
		@mkdir($blocked, 0700, true);
		file_put_contents($blocked . '/child', 'x');
		$GLOBALS['gpsmap_stub_log'] = [];
		...
		assert_equal('write: a failed rename leaves no temp file', [],
			preg_grep('/occupied2\..*\.tmp$/', scandir(gpsmap_test_tmpdir() . '/plugins/gpsmap/XML')));
	});
...
it('gpsmap_poller_bottom(): the production entry point, DNS worker and health checks', function () use ($root) {
