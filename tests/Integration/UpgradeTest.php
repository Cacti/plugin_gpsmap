...
/*
 ...
 +-------------------------------------------------------------------------+
 | Runs in an isolated child process: the install/upgrade lifecycle needs   |
 | its own db_execute()/set_config_option()/etc. stubs that differ from the |
 | generic ones in bootstrap-unit.php, and those are only valid here.       |
 +-------------------------------------------------------------------------+
*/

it('exercises the plugin install and upgrade lifecycle', function () {
	$probe = <<<'PROBE'
	<?php
	$GLOBALS['gpsmap_probe_pass'] = 0;
	$GLOBALS['gpsmap_probe_fail'] = 0;

	function assert_equal($label, $expected, $actual) {
		if ($expected === $actual) {
			echo "PASS  $label\n";
			$GLOBALS['gpsmap_probe_pass']++;
		} else {
			echo "FAIL  $label\n";
			echo '      expected: ' . var_export($expected, true) . "\n";
			echo '      actual:   ' . var_export($actual, true) . "\n";
			$GLOBALS['gpsmap_probe_fail']++;
		}
	}
	 ...
	 * same store read_config_option reads. These override bootstrap-unit.php's
	 * generic stubs because they are declared before it is required below. */
	...
	require_once $root . '/includes/setup/database.php';
	...
	fwrite(STDOUT, "\n" . $GLOBALS['gpsmap_probe_pass'] . " passed, " . $GLOBALS['gpsmap_probe_fail'] . " failed\n");
	exit($GLOBALS['gpsmap_probe_fail'] > 0 ? 1 : 0);
	PROBE;

	$file = gpsmap_test_tmpdir() . '/upgrade_probe.php';
	...
	expect($status)->toBe(0, "upgrade lifecycle probe failed:\n" . implode("\n", $output));
});
